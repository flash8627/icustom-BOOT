...
public interface SysUserDao extends JpaRepository<SysUser, Integer> {
	...
	@Query("select u from SysUser u where u.account=:account")
	SysUser findByName(@Param("account") String account);
	...
	@Modifying
	@Query("update SysUser u set u.username = ?1, u.email = ?2, u.password = ?3 where u.id = ?4")
